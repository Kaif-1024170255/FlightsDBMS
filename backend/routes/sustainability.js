...
router.get('/metrics', async (req, res) => {
    try {
        ...
        const metrics = {
            total_co2: co2Res[0].total_co2 || 0,
            total_recycled: wasteRes[0].total_recycled || 0,
            total_waste: wasteRes[0].total_waste || 0,
            avg_score: avgScoreRes[0].avg_score ? parseFloat(avgScoreRes[0].avg_score).toFixed(2) : 0,
        };
        
        // Calculate recycling rate
        metrics.recycling_rate = (metrics.total_waste + metrics.total_recycled) > 0 
            ? ((metrics.total_recycled / (metrics.total_waste + metrics.total_recycled)) * 100).toFixed(1)
            : 0;
